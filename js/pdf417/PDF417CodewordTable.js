 ...
 *
 * Official PDF417 codeword symbol patterns.
 *
 * 3 clusters × 929 codewords.
 *
 * Each value is a 17-bit bar/space pattern.
 */
class PDF417CodewordTable {
    ...
    static TABLE = [
        PDF417CodewordTable.CLUSTER_0,
        PDF417CodewordTable.CLUSTER_3,
        PDF417CodewordTable.CLUSTER_6
    ];

    static getPattern(codeword, cluster) {

        if (
                !Number.isInteger(codeword) ||
                codeword < 0 ||
                codeword > 928
                ) {
        ...
        switch (cluster) {
            ...
            default:
                throw new RangeError(
                        "PDF417 cluster must be 0, 1, 2 " +
                        "or specification cluster 0, 3, 6."
                        );
        }

        return PDF417CodewordTable.TABLE[
                clusterIndex
        ][codeword];
    }
    ...
    static validate() {
        ...
        if (
                !Array.isArray(PDF417CodewordTable.TABLE) ||
                PDF417CodewordTable.TABLE.length !== 3
                ) {
            throw new Error(
                    "PDF417 requires exactly three codeword clusters."
                    );
        }
        ...
        for (
            ...
            const table =
                    PDF417CodewordTable.TABLE[cluster];
            ...
            if (
                ...
                throw new Error(
                        "PDF417 cluster " +
                        cluster +
                        " must contain exactly 929 patterns."
                        );
            }
            ...
            for (
                ...
                if (
                    ...
                    throw new Error(
                            "Invalid PDF417 pattern at cluster " +
                            cluster +
                            ", codeword " +
                            codeword +
                            "."
